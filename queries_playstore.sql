...
--SELECT category, COUNT(category) AS num_of_apps FROM analytics GROUP BY category ORDER BY category;
...
--SELECT app_name, rating, reviews FROM analytics WHERE rating < 3 AND reviews > 10000 ORDER BY reviews DESC;
...
--SELECT category, COUNT(app_name) AS num_of_apps FROM analytics GROUP BY category HAVING COUNT(app_name) > 300 ORDER BY COUNT(app_name);
...
--SELECT app_name,reviews, min_installs, (min_installs/reviews) AS proportion_of_installs_per_review FROM analytics WHERE min_installs >= 100000 ORDER BY  proportion_of_installs_per_review DESC LIMIT 1;





EXAMPLE OF analytics DB:
id              | 2
